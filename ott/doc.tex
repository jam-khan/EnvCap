...
\begin{align*}
  ...
  % &\text{Ordinary types} &A^\circ,B^\circ,C^\circ ::=&~  \top  ~|~  \bot  ~|~  \mathbb{Z}  ~|~ \ottmv{X} ~|~ \ottnt{A}  \rightarrow  B^\circ ~|~  \forall  \ottmv{X} \!*\! \ottnt{A} .\; B^\circ  ~|~ \ottsym{\{}  \ell  \ottsym{:}  A^\circ  \ottsym{\}} \\
