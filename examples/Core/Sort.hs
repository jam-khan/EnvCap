...
lessthanEqual :: Exp
lessthanEqual =  Fix (Lam (TPair TInt (TList TInt))
                            (LCase (Snd (proj 0))
                                    (Nil TInt)
...
greaterthan :: Exp
greaterthan =  Fix (Lam (TPair TInt (TList TInt))
                            (LCase (Snd (proj 0))
                                    (Nil TInt)
...
merge :: Exp
merge = Fix (Lam (TPair (TList TInt) (TList TInt))
                (LCase  (Fst (proj 0))
                        (Snd (proj 0))
                        (Cons   (proj 1)
...
split :: Exp
split = Lam (TArrow (TPair TInt (TList TInt)) (TList TInt))
                (Lam    (TPair TInt (TList TInt))
                        (App (proj 1) (proj 0)))

sort :: Exp
sort = Fix (Lam (TList TInt)
                (LCase  (proj 0)
                        (Nil TInt)
                        (App merge      (Pair   (App (proj 3) (App (App split lessthanEqual)  (Pair (proj 1) (proj 0))))
                                                (Cons   (proj 1)
                                                        (App    (proj 3)
                                                                (App (App split greaterthan)    (Pair (proj 1) (proj 0)))))))))


temp' :: Exp
temp' = App sort (Cons (Lit 1) (Cons (Lit 5) (Cons (Lit 10) (Nil TInt))))
