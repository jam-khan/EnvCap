...
infer ctx (Proj e n)        = do
    ...
    case lookupt tB n of
        Just t -> Right t
        Nothing -> Left $ TypeError $ "Projection " ++ show n ++ " failed on type " ++ show tB ++ " ctx: " ++ show ctx
-- TYP-LIT
...
infer ctx (App e1 e2)       = do
    tE1 <- infer ctx e1
    case tE1 of
        TArrow tA tB -> if check ctx e2 tA then Right tB else Left $ TypeError "Type mismatch in application"
...
infer ctx (If cond e1 e2)   = do 
    if check ctx cond TBool then do 
        t1 <- infer ctx e1 
        t2 <- infer ctx e2 
        if t1 == t2 then Right t1 else Left $ TypeError "Branches of if must have the same type"
...
-- TYP-NIL 
infer ctx (Nil ta)          = Right $ TList ta  
-- TYP-CONS 
infer ctx (Cons te te2)     = do  
    tt2 <- infer ctx te2  
    case tt2 of  
        TList ta -> if check ctx te ta then Right tt2 else Left $ TypeError "Type mismatch in list constructor"
        _ -> Left $ TypeError "Expected a list type"
-- TYP-LCASE 
...
infer ctx (BinOp (Arith _) e1 e2) = do
                                        ti <- infer ctx e1
                                        t2 <- infer ctx e2
                                        if ti == TInt && check ctx e2 TInt  then Right TInt 
