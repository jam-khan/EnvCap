...
infer ctx (BinOp (Comp _) e1 e2) = case infer ctx e1 of
    ...
    _ -> Nothing
{--
(Mrg Ctx (Lam (TAnd TString TUnit) Ctx))
    ? ,, (\x:(String & Unit). ?)
    ==> (TAnd Unit Unit)
    Unit ,, TArrow (String & Unit) (TAnd TUnit (TAnd String TUnit))
    (TAnd TUnit (TArrow (TAnd TString TUnit) (TAnd ))
    (TAnd TUnit (TArrow (TAnd TString TUnit) (TAnd (TAnd TUnit TUnit) (TAnd TString TUnit)))) 
--}
-- TYP-LOGIC
...
prop_progress t = counterexample (show t) $
    case infer TUnit t of
        Just ty -> case evalBig VUnit t of
